...
/// every owned/borrowed operand combination from an
/// [`ops::BinaryOp`](crate::ops::BinaryOp) implemented by the marker type `$op`.
///
/// `$trait`/`$method` and `$assign_trait`/`$assign_method` are the operator and assigning-operator
/// traits (e.g. `BitAnd`/`bitand` and `BitAndAssign`/`bitand_assign`); they must be in scope at
/// the call site.
